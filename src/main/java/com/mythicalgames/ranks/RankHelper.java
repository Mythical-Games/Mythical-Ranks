...
public class RankHelper {
    ...
    public void registerAllGroups(Config config) {
        ...
        if (defaultGroup != null) {
            ...
            if (RankSystem.getInstance().config.debug) log.info("Default group set to '{}'", defaultGroup.getName());
        }

        if (RankSystem.getInstance().config.debug) log.info("Loaded {} permission groups from config.", loaded.size());
    }
    ...
    public CompletableFuture<Void> setPlayerGroup(EntityPlayer player, Config config, String groupName) {
        ...
        if (RankSystem.getInstance().config.debug) log.info("Player {} assigned to group '{}'", player.getOriginName(), targetGroup.getName());
    ...
    public boolean createAndSaveGroup(Config config, String groupName, @Nullable String parentGroup) {
        ...
        if (RankSystem.getInstance().config.debug) log.info("Saved new group '{}' to config.", groupName);
        return true;
    }
